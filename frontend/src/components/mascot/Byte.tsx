...
/** Byte — o mascote robô do Verbus. Reage ao desempenho do usuário. */
  ...
  return (
    <motion.div className={className} animate={bodyAnim[state]} style={{ width: size, height: size }}>
      <svg viewBox="0 0 200 200" width="100%" height="100%" aria-label="Byte, o mascote">
        ...
        {/* antena */}
        <line x1="100" y1="40" x2="100" y2="20" stroke="#0F2A4A" strokeWidth="5" strokeLinecap="round" />
        <motion.circle
          cx="100"
          cy="16"
          r="7"
        ...
        {/* corpo */}
        <rect x="46" y="40" width="108" height="96" rx="26" fill="#1C4A7A" />
        <rect x="46" y="40" width="108" height="96" rx="26" fill="url(#shine)" opacity="0.18" />

        {/* visor */}
        <rect x="60" y="58" width="80" height="56" rx="18" fill="#0A1E36" />

        {/* olhos */}
        {isSleep ? (
          <>
            <path d="M72 86 q10 8 20 0" stroke="#F5A524" strokeWidth="4" fill="none" strokeLinecap="round" />
            <path d="M108 86 q10 8 20 0" stroke="#F5A524" strokeWidth="4" fill="none" strokeLinecap="round" />
            <text x="138" y="56" fontSize="18" fill="#F5A524">z</text>
          </>
        ) : (
          <>
            <motion.circle
        ...
        {talking ? (
          <motion.ellipse
            cx="100" cy="102" rx="13" fill="#F5A524"
            animate={{ ry: [3, 9, 4, 8, 3] }}
            transition={{ duration: 0.5, repeat: Infinity, ease: 'easeInOut' }}
            style={{ transformOrigin: '100px 102px' }}
          />
        ) : isSad ? (
          <path d="M84 104 q16 -10 32 0" stroke="#F5A524" strokeWidth="4" fill="none" strokeLinecap="round" />
        ) : eyeMouth ? (
          <path d="M82 100 q18 16 36 0" stroke="#F5A524" strokeWidth="4" fill="none" strokeLinecap="round" />
        ) : (
          <path d="M84 102 h32" stroke="#F5A524" strokeWidth="4" fill="none" strokeLinecap="round" />
        )}

        {/* braços */}
        <motion.rect
          x="30" y="74" width="16" height="40" rx="8" fill="#0F2A4A"
          animate={state === 'cheer' || state === 'celebrate' ? { rotate: [-30, -50, -30] } : {}}
          transition={{ duration: 0.6, repeat: Infinity }}
          style={{ transformOrigin: '38px 80px' }}
        />
        <motion.rect
          x="154" y="74" width="16" height="40" rx="8" fill="#0F2A4A"
          animate={state === 'cheer' || state === 'celebrate' ? { rotate: [30, 50, 30] } : {}}
          transition={{ duration: 0.6, repeat: Infinity }}
        ...
        {/* pés */}
        <rect x="64" y="136" width="26" height="20" rx="8" fill="#0F2A4A" />
        <rect x="110" y="136" width="26" height="20" rx="8" fill="#0F2A4A" />
